...
#########################Code Here###############################################
    ...
    #         Otherwise return False
    def compare_pws(self, pw, duplicate):
        return pw == duplicate
    ...
    def gen_token(self):
        key = os.urandom(20)
        self.totp = TOTP(key, 8, SHA1(), 300)
        time_value = time.time()
        token = self.totp.generate(time_value)
        return token.decode()
    ...
    #        Method self.store_credentials used to store username, pw, and salt
    ...
    def scrypt_derive(self, username, pw):
        ...
        pw = kdf.derive(pw.encode())
        self.store_credentials(username, pw, salt)
